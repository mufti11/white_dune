...
// MyMesh::generateTextureCoordinates() based on Polyrep.c of FreeWRL
/*******************************************************************
...
MyMesh::MyMesh(MeshBasedNode *node,
...
MyMesh::copy(void)
{
    ...
    MyMesh *mesh = new MyMesh(m_node, 
                              (MFVec3f *)m_vertices->copy(), 
...
MyMesh::draw(int pass)
{
    ...
    const float *vertices = m_vertices->getValues();
...
MyMesh::generateFaceNormals()
{
    ...
    if (m_vertices->getSize() != 0) {
        for (int i = 0; i < n; face++) {
            ...
            Vec3f v1 = c1 - c2;
            Vec3f v2 = c3 - c2;
            Vec3f normal = m_ccw ? v2.cross(v1) : v1.cross(v2);
            if (normal.isZero())
                normal = c2;
...
MFVec2f* 
MyMesh::generateTextureCoordinates()
{
    m_texcoords_generated = true;
    return ::generateTextureCoordinates(m_vertices, m_coordIndex); 
}

#define VEC3F(x,y) Vec3f((*x).getValue(y))

bool 
MyMesh::onlyPlanarFaces(void)
{
    int vertexCounter = 0;
...
void
MyMesh::setNormals(MFVec3f *normals) 
{
    if (m_normals)
        m_normals->unref();
    m_normals = normals;
...
MFInt32 *
MyMesh::optimizeCoordIndex(void)
{
    const float *vertices = m_vertices->getValues();
    MFInt32 *coordIndex = new MFInt32();
    for (int i = 0; i < m_coordIndex->getSize(); i++)
...
void
MyMesh::optimizeVertices(MFVec3f **newCoord, MFInt32 **newCoordIndex, 
                       float epsilon)
{
...
int                 
MyMesh::getMaxNumberEdgesPerFace(void)
{
...
void MyMesh::appendIndices(MFInt32 *indices, MFInt32 *index, int offset, 
                           int appendList[8])
{
    for (int i = 0; i < 8; i++)
        if (appendList[i] > -1)
...
void
MyMesh::simpleQuadTriangulate(void)
{
    ...
    for (int i = 0; i < getNumFaces(); i++) {
        ...
        if (m_faces[i]->getNumVertices() == 4) {
            const float *vertices = m_vertices->getValues();
            Vec3f v1(vertices + m_coordIndex->getValue(offset + 0) * 3);
            Vec3f v2(vertices + m_coordIndex->getValue(offset + 1) * 3);
            Vec3f v3(vertices + m_coordIndex->getValue(offset + 2) * 3);
            Vec3f v4(vertices + m_coordIndex->getValue(offset + 3) * 3);
            // account length
...
void                
MyMesh::addCoords(MFInt32 *coords, float *color, MFFloat *colors, 
                  bool rgbaColor, MFVec3f *normals, MFVec2f *texCoords)
{
    if (m_coordIndex == NULL)
        return;
...
MyLine
MyMesh::getLine(int line)
{
    return m_lines[line];
}

int
MyMesh::getMeshFlags(void) {
    int meshFlags = 0;
...
void
MyMesh::setMeshFlags(int meshFlags) {
...
int 
MyMesh::countPolygons2Sided(void)
{
    return m_node->isDoubleSided() ? 2 * countPolygons() : 0;
}

int 
MyMesh::countPolygons1Sided(void)
{
    return m_node->isDoubleSided() ? 0 : countPolygons();
}

Node * 
MyMesh::toIndexedFaceSet(int meshFlags, Scene *scene)
{
...
MFInt32 *
MyMesh::getSmoothNormalIndex(void)
{
...
#else
void                
MyMesh::startTessellation(GLenum type)
{
...
void 
MyMesh::addNewVertex(VertexInfo* vertexInfo)
{
...
void MyMesh::collectMemoryToFree(VertexInfo * vertexInfo)
{
...
void 
MyMesh::initialiseTriangulation(GLUtesselator *tess)
{
...
{
    MFVec3f *vertices = new MFVec3f(m_triangulatedVertices.size() / 3);
...
MyMesh::triangulate(MeshBasedNode *that)
{
    ...
    for (int i = 0; i < m_numFaces; i++) {
        ...
        for (j = offset; j < offset + numVertices; j++) {
            ...
            const float *vertices = m_vertices->getValue(coordIndex[j]);
