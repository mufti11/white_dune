...
ElevationGridDialog::SaveData()
{
    ...
    swGetText(swGetDialogItem(m_dlg, IDC_WIDTH), buf, 128);
    m_width = atoi(buf);
    swGetText(swGetDialogItem(m_dlg, IDC_DEPTH), buf, 128);
    m_depth = atoi(buf);
}
...
ElevationGridDialog::LoadData()
{
    ...
    mysnprintf(buf, 128, "%d", m_width);
    swSetText(swGetDialogItem(m_dlg, IDC_WIDTH), buf);
    mysnprintf(buf, 128, "%d", m_depth);
    swSetText(swGetDialogItem(m_dlg, IDC_DEPTH), buf);
}
