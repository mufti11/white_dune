...
TYPE white_dune_vrml_editor
   LEGEND white_dune executable (stereoscopic view enabled on a stereo visual) "dune DUNEOPTIONS"
...
TYPE white_dune_vrml_editor_no_stereo
   LEGEND script to start white_dune with stereoscopic view disabled: "dune DUNEOPTIONS"
   MATCH tag==0x00ff1a01;
   SUPERTYPE Script
   DROPIF VRML97 Vrml2.0 Vrml2.0File Vrml2.0DataFile
   CMD OPEN $LEADER -nostereo DUNEOPTIONS $REST
   CMD DROP $TARGET DUNEOPTIONS $SELECTED
   ICON  {
             if (opened) {
                 include("iconlib/Dunebackground.fti");
