...
./configure --with-uninstallcomment="find and remove directory/application white_dune.app" --with-optimization --without-devil --with-helpurl="http://wdune.ourproject.org/docs" && (cd src/SDLjoystick && make libSDLjoystick.a) && (cd src && make mac_dune) &&
