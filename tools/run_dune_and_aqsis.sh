...
if test -x /usr/bin/mencoder ; then
    ...
    mencoder "mf://`ls $FILE*.tif | sort -V | awk '{printf $0 ","}' `" -o $FILE.mp4 -nosound -mf type=tif f:fps=24 -of lavf -lavfopts format=mp4 -ovc x264 -x264encopts pass=1:bitrate=2000:crf=24 &&
