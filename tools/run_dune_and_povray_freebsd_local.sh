...
if test X_$# = X_0 ; then 
   ...
   echo '# attension: removes file*.pov and file*.png' 2>&1 
   exit 1
fi
if test -f $1 ; then 
   echo > /dev/null 
...
NPROC=`sysctl hw.model hw.machine hw.ncpu | \
awk '/hw.ncpu/ { print $2 * <system("sysctl -n hw.ncpu")}'`
...
if test -x "`which mencoder`" ; then
    ...
    NUMFAMES=`ls $FILE*.png | wc -l`
    NUMDIGITS=`echo $NUMFRAMES | wc -c`
    png2yuv -I p -f 24 -b 1 -n $NUMFAMES -j "$FILE$NUMDIGITS%d.png" > $FILE.yuv
    if test -x "`which vpxenc`" ; then
        vpxenc --good --cpu-used=$NPROC --auto-alt-ref=1 --lag-in-frames=16 --end-usage=vbr --passes=2 --threads=2 --target-bitrate=3000 -o $FILE.webm $FILE.yuv &&
        rm $FILE.yuv $FILE*.png
    else
        animate -alpha off `ls $FILE*.png $FILE*.tif | sort -V | awk '{printf $2 " "}'`
    fi
fi
