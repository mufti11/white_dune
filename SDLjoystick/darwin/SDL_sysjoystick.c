...
struct recElement
{
    IOHIDElementCookie cookie; // unique value which identifies element, will 
                               // NOT change
...
/*
    ...
    Boolean wrapping;          // does element wrap around (one value higher 
                               //                           than max is min)
    Boolean nonLinear;         // are the values reported non-linear relative 
                               // to element movement
    Boolean preferredState;    // does element have a preferred state 
                               // (such as a button)
...
{
    IOHIDDeviceInterface ** interface;  // interface to device, 
                                        // NULL = no interface

    char product[256];                  // name of product
    long usage;                         // usage page from IOUSBHID Parser.h 
                                        // which defines general usage
...
#ifdef DEBUG
                                fprintf(stderr,"minReport old value %ld\n",pElement->minReport);
#endif
                pElement->minReport = hidEvent.value;
#ifdef DEBUG
                                fprintf(stderr,"minReport new value %ld\n",pElement->minReport);
#endif
                        }
            if (hidEvent.value > pElement->maxReport) {
#ifdef DEBUG
                                fprintf(stderr,"maxReport old value %ld\n",pElement->maxReport);
#endif
                pElement->maxReport = hidEvent.value;
#ifdef DEBUG
                                fprintf(stderr,"maxReport new value %ld\n",pElement->maxReport);
#endif
                        }
...
static void HIDGetElementInfo (CFTypeRef refElement, recElement *pElement)
{
    ...
    if (refType && CFNumberGetValue (refType, kCFNumberLongType, &number)) {
        pElement->min = number;
        pElement->minReport = pElement->min;
        }
    refType = CFDictionaryGetValue (refElement, CFSTR(kIOHIDElementMaxKey));
    if (refType && CFNumberGetValue (refType, kCFNumberLongType, &number)) {
        pElement->max = number;
        pElement->maxReport = pElement->max;
        }
/*
    TODO: maybe should handle the following stuff somehow?
